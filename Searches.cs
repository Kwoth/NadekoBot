            ...
            manager.CreateCommands("", cgb => {
                ...
                cgb.CreateCommand("~#")
                  ...
                  .Do(async e => {
                      ...
                      await e.Channel.SendIsTyping();
                      var res = await SearchHelper.GetResponseAsync($"https://tagdef.p.mashape.com/one.{Uri.EscapeUriString(arg)}.json",
                          new Tuple<string, string>[] {
                                  new Tuple<string, string>("X-Mashape-Key", NadekoBot.creds.MashapeKey)
                          });
                      try
                      {
                          var items = JObject.Parse(res);
                          await e.Send($"`Hashtag:` {items["defs"]["def"]["hashtag"].ToString()}\n");
                          await e.Send($"`Definition:` {items["defs"]["def"]["text"].ToString()}\n");
                          await e.Send($"`Link:` <{await items["defs"]["def"]["uri"].ToString().ShortenUrl()}>");
                      }
                      catch (Exception ex)
                      {
                          await e.Channel.SendMessage("💢 Exception: " + ex.Message);
