...
Version: `{NadekoStats.Instance.BotVersion}`";
        ...
        internal override void Init(CommandGroupBuilder cgb)
        {
            cgb.CreateCommand(Module.Prefix + "h")
                ...
                .Description($"Either shows a help for a single command, or PMs you help link if no arguments are specified. | `{Module}h !m q` or just `{Prefix}h`")
...
**LIST OF COMMANDS**: <https://github.com/Kwoth/NadekoBot/blob/master/commandlist.md>").ConfigureAwait(false));
            ...
            cgb.CreateCommand(Module.Prefix + "donate")
                .Alias("~donate")
                .Description("Instructions for helping the project! | `-donate` or `~donate`")
                .Do(async e =>
                {
                    await e.Channel.SendMessage(
