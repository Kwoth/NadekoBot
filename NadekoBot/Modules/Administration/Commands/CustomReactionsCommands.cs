    ...
    class CustomReactionsCommands : DiscordCommand
    {
        ...
        internal override void Init(CommandGroupBuilder cgb)
        {
            ...
            cgb.CreateCommand(Prefix + "listcustreact")
                .Alias(Prefix + "lcr")
                .Description($"Lists custom reactions (paginated with 30 commands per page). Use 'all' instead of page number to get all custom reactions DM-ed to you.  |`{Prefix}lcr 1` or `{Prefix}lcr all` ")
            ...
            cgb.CreateCommand(Prefix + "delcustreact")
                .Alias(Prefix + "dcr")
                .Description($"Deletes a custom reaction with given name (and index) | {Prefix}dcr Reaction 1")
                .Parameter("name", ParameterType.Required)
