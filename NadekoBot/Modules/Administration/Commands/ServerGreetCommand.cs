    ...
    internal class ServerGreetCommand : DiscordCommand
    {
        ...
        internal override void Init(CommandGroupBuilder cgb)
        {
            cgb.CreateCommand(Module.Prefix + "grdel")
                .Description($"Toggles automatic deletion of greet and bye messages. | `{Module.Prefix}grdel`")
                .Do(async e =>
                {
            ...
            cgb.CreateCommand(Module.Prefix + "greet")
                .Description($"Toggles anouncements on the current channel when someone joins the server. | `{Module.Prefix}greet`")
                .Do(async e =>
                {
                    if (!e.User.ServerPermissions.ManageServer) return;
                    var ann = AnnouncementsDictionary.GetOrAdd(e.Server.Id, new AnnounceControls(e.Server.Id));
            ...
            cgb.CreateCommand(Module.Prefix + "greetmsg")
                .Description($"Sets a new join announcement message. Type %user% if you want to mention the new member. Using it with no message will show the current greet message. | `{Module.Prefix}greetmsg Welcome to the server, %user%.`")
            ...
            cgb.CreateCommand(Module.Prefix + "bye")
                .Description($"Toggles anouncements on the current channel when someone leaves the server. | `{Module.Prefix}bye`")
                .Do(async e =>
                {
                    if (!e.User.ServerPermissions.ManageServer) return;
                    var ann = AnnouncementsDictionary.GetOrAdd(e.Server.Id, new AnnounceControls(e.Server.Id));
            ...
            cgb.CreateCommand(Module.Prefix + "byemsg")
                .Description($"Sets a new leave announcement message. Type %user% if you want to mention the new member. Using it with no message will show the current bye message. | `{Module.Prefix}byemsg %user% has left the server.`")
                .Parameter("msg", ParameterType.Unparsed)
                .Do(async e =>
                {
                    if (!e.User.ServerPermissions.ManageServer) return;
            ...
            cgb.CreateCommand(Module.Prefix + "byepm")
                .Description($"Toggles whether the good bye messages will be sent in a PM or in the text channel.| `{Module.Prefix}byepm`")
                .Do(async e =>
                {
                    if (!e.User.ServerPermissions.ManageServer) return;
                    var ann = AnnouncementsDictionary.GetOrAdd(e.Server.Id, new AnnounceControls(e.Server.Id));
            ...
            cgb.CreateCommand(Module.Prefix + "greetpm")
                .Description($"Toggles whether the greet messages will be sent in a PM or in the text channel.| `{Module.Prefix}greetpm`")
                .Do(async e =>
                {
                    if (!e.User.ServerPermissions.ManageServer) return;
