    ...
    internal class IncidentsCommands : DiscordCommand
    {
        ...
        internal override void Init(CommandGroupBuilder cgb)
        {
            cgb.CreateCommand(Module.Prefix + "listincidents")
                .Alias(Prefix + "lin")
                .Description($"List all UNREAD incidents and flags them as read.| `{Module.Prefix}lin`")
            ...
            cgb.CreateCommand(Module.Prefix + "listallincidents")
                .Alias(Prefix + "lain")
                .Description($"Sends you a file containing all incidents and flags them as read.| `{Module.Prefix}lain`")
                .AddCheck(SimpleCheckers.ManageServer())
                .Do(async e =>
                {
                    var sid = (long)e.Server.Id;
